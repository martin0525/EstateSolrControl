        ...
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                string sql = @"SELECT [MainID]
                            ...
                            ,[DataSource]
                            ,[UpdateTime]
                            FROM [dbo].[TB_ESTATE_BASEINFO_FULLINDEX_V3] where state =1 ";
                sql += " and UpdateTime >(select max([LastUpdateTime]) from [dbo].[TB_ESTATE_BASE_FULLIND_UPDATE_INFO])";

                Console.WriteLine("读取数据源。。。");

                System.Data.DataSet result = SimpleDataHelper.Query(SimpleDataHelper.CoreCenterConnectionString, sql);

                var LastTime = (from t in result.Tables[0].AsEnumerable() select t.Field<DateTime>("UpdateTime")).Max();
                ...
                SimpleDataHelper.UpdateRecordTime(SimpleDataHelper.CoreCenterConnectionString, LastTime);
